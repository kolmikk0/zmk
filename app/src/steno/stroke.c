...
void find_strokes(const uint8_t *strokes, const uint8_t len, const uint8_t free) {
    ...
    for (; ; bucket_ind += BUCKET_SIZE) {
        steno_flash_read(bucket_ind, (uint8_t *) &last_bucket, BUCKET_SIZE);
#ifdef STENO_DEBUG_STROKE
        ...
        steno_flash_read(byte_ptr, entry_buf, byte_len);
#ifdef STENO_DEBUG_STROKE
        steno_debug("      strokes: ");
        for (uint8_t i = 0; i < len; i ++) {
        ...
        if (memcmp(strokes, entry_buf, byte_len) == 0) {
            ...
            steno_flash_read(byte_ptr + byte_len, entry_buf + byte_len, entry_len + 1);
            return;
        } else {
            continue;
        }
...
void read_entry(uint32_t bucket) {
    ...
    steno_flash_read(byte_ptr, entry_buf, strokes_len * STROKE_SIZE + 1 + entry_len);
}
